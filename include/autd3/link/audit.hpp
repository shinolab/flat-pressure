...
class Audit final {
 ...
 public:
  ...
  AUTD3_API [[nodiscard]] uint8_t debug_output_idx(const size_t idx) const {
    return AUTDLinkAuditFpgaDebugOutputIdx(_ptr, static_cast<uint32_t>(idx));
  }
  ...
  AUTD3_API [[nodiscard]] std::vector<std::uint8_t> modulation(const size_t idx, const native_methods::Segment segment) const {
  ...
  AUTD3_API [[nodiscard]] uint32_t modulation_frequency_division(const size_t idx, const native_methods::Segment segment) const {
    return AUTDLinkAuditFpgaModulationFrequencyDivision(_ptr, segment, static_cast<uint32_t>(idx));
  }

  AUTD3_API [[nodiscard]] driver::LoopBehavior modulation_loop_behavior(const size_t idx, const native_methods::Segment segment) const {
    return driver::LoopBehavior(AUTDLinkAuditFpgaModulationLoopBehavior(_ptr, segment, static_cast<uint32_t>(idx)));
  }

  AUTD3_API [[nodiscard]] std::pair<std::vector<std::uint8_t>, std::vector<std::uint8_t>> drives(const size_t idx,
  ...
  AUTD3_API [[nodiscard]] uint32_t stm_frequency_division(const size_t idx, const native_methods::Segment segment) const {
    return AUTDLinkAuditFpgaStmFrequencyDivision(_ptr, segment, static_cast<uint32_t>(idx));
  }

  AUTD3_API [[nodiscard]] driver::LoopBehavior stm_loop_behavior(const size_t idx, const native_methods::Segment segment) const {
  ...
  AUTD3_API [[nodiscard]] std::vector<std::uint8_t> phase_filter(const size_t idx) const {
    const auto n = AUTDLinkAuditCpuNumTransducers(_ptr, static_cast<uint32_t>(idx));
    std::vector<std::uint8_t> buf(n);
