...
#define AUTD3_DEF_PARAM_INTENSITY(T, PARAM_NAME)                                                           \
  AUTD3_API void with_##PARAM_NAME(const uint8_t value)& { _##PARAM_NAME = driver::EmitIntensity(value); } \
  AUTD3_API [[nodiscard]] T&& with_##PARAM_NAME(const uint8_t value)&& {                                   \
    _##PARAM_NAME = driver::EmitIntensity(value);                                                          \
    return std::move(*this);                                                                               \
  }                                                                                                        \
  AUTD3_API void with_##PARAM_NAME(const driver::EmitIntensity value)& { _##PARAM_NAME = value; }          \
  AUTD3_API [[nodiscard]] T&& with_##PARAM_NAME(const driver::EmitIntensity value)&& {                     \
    _##PARAM_NAME = value;                                                                                 \
    return std::move(*this);                                                                               \
  }                                                                                                        \
                                                                                                           \
  AUTD3_API [[nodiscard]] driver::EmitIntensity PARAM_NAME() const { return _##PARAM_NAME; }               \
                                                                                                           \
 private:                                                                                                  \
...
AUTD3_API constexpr auto validate(T res) {
...
AUTD3_API constexpr T validate(ResultI32 res) {
...
AUTD3_API static inline SamplingConfiguration validate(ResultSamplingConfig res) {
  const auto [result, err_len, err] = res;
  if (result.div == 0) {
    const std::string err_str(err_len, ' ');
    AUTDGetErr(err, const_cast<char*>(err_str.c_str()));
